...
class SpellChecker{
		...
		//	cout<<dict.search("eat");
			...
			void SpellCorrector(string s1){
				...
				int distance=1;
			    while(words<10){
			    	if(!similar(s1,dict,distance).empty()){
			    		matchingWords[i]=similar(s1,dict,distance);
			    		i++;
			    		words++;
				...
				cout<<"Did you mean one of the following words? "<<endl;
				for(int i=0;i<=9;i++){
...
//				}
//			}
 string similar(string s,BST bst,int dis)
    {
        treeNode* t=bst.root;
        int i=0;
        while (t!=NULL){
...
int main(){
	...
	SpellChecker sc;
	sc.Dictionary();
	sc.Preprocessing("quickly");
   cout<< sc.distance("halew","hello")<<endl;
   sc.SpellCorrector("appl");
